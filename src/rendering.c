...
void RENDER_RenderMap(Game *game, SDL_FPoint camera_pos) {
    ...
    int j_end = ((int) camera_pos.x + window_w) / TILE_SIZE + 1;
    int i_end = ((int) camera_pos.y + window_h) / TILE_SIZE + 1;

    SDL_Rect dst = {0, 0, TILE_SIZE, TILE_SIZE};
    for(int i = ((int) (camera_pos.y) / TILE_SIZE);i<i_end; i++){
        for(int j = ((int) (camera_pos.x) / TILE_SIZE);j<j_end; j++){
            dst.x = TILE_SIZE*j - camera_pos.x;
            dst.y = TILE_SIZE*i - camera_pos.y;
    ...
    for(int i=0;i<HAUTEUR;i++){
        for(int j=0;j<LARGEUR;j++){
            ...
            if(dst.x+TILE_SIZE<0 || dst.y+TILE_SIZE<0) continue;
